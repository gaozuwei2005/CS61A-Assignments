...
def about(subject):
    ...
    def about_func(paragraph):
        paragraph = lower(remove_punctuation(paragraph)).split(' ')
...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        return 100.0
    if (len(typed_words) == 0) ^ (len(source_words) == 0):
        return 0.0
    i = 0
    correct_count = 0;
    while i < len(typed_words) and i < len(source_words):
        if typed_words[i] == source_words[i]:
            correct_count += 1
    return correct_count / len(typed_words)
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if diff < min_diff:
            target, min_diff = word, diff
    return target if min_diff <= limit else typed_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    if limit < 0:
        ...
        return (typed[0] != source[0]) + furry_fixes(typed[1:], source[1:], limit - (typed[0] != source[0]))
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    max_pos = 0
    while max_pos < min(len(typed), len(source)) and typed[max_pos] == source[max_pos]:
        max_pos += 1
    progress = max_pos / len(source)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[t[i + 1] - t[i] for i in range(0, len(t) - 1)] for t in tpp]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest = [[] for i in range(0, player_indices)]
    for word_id in range(0, word_indices):
        best_player = 0
        for now_player in range(1, player_indices):
            if times[now_player][word_id] < times[best_player][word_id]:
                best_player = now_player
        fastest[best_player].append(words[word_id])
    return fastest
    # END PROBLEM 10
