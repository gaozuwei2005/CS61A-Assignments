...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        cur_args = args
        while cur_args:
            args_list.append(cur_args.first)
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        return eval_all(procedure.body, procedure.env.parent
                        parent                make_child_frame(procedure.formals, args))
        # END PROBLEM 11
    else:
...
def eval_all(expressions, env):
    ...
    current_val = None
    while current_exp:
        current_val = scheme_eval(current_exp.first, env)
