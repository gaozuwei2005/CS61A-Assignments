...
def is_always_roll(strategy, goal=GOAL):
    ...
    n = strategy(0, 0)
    is_equal = True
    while score < goal and is_equal:
        opponent_score = 0
        while opponent_score < goal and is_equal:
            if n != strategy(score, opponent_score):
                is_equal = False
            opponent_score += 1
        score += 1
    return is_equal
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    "*** YOUR CODE HERE ***"
    max_dice = 1
    max_average = make_averaged(always_roll(1), times_called)(1, )
    now_dice = 2
    while now_dice <= 10:
        now_average = make_averaged(always_roll(now_dice), times_called)()
        if now_average > max_average:
            max_dice = now_dice
            max_average = now_average
        now_dice += 1
    return max_dice
    # END PROBLEM 9
