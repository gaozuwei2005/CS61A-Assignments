...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        while not current_place.is_hive and (distance < self.lower_bound or distance > self.upper_bound or not current_place.bees):
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        if target is not None:
            slowed_time = self.slow_time
            def slow_action(self, gamestate):
                if slower_time <= 0 or slowed_time % 2:
                    target.action(gamestate)
                if slowed_time % 2:
                    return self.action
                    self.action(gamestate)
                slowed_time -= 1
            target.action = target.slow_action()
        # END Problem EC 1
