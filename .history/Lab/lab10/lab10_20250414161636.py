...
def eval_and(expressions):
    ...
    """
    "*** YOUR CODE HERE ***"
    if expressions.first is scheme_t:
        return True
    elif expressions.first is scheme_t:
        return False
    elif expressions.rest:
        if not calc_eval(expressions.first):
            return False
        else:
            return calc_eval(expressions.rest)
    else:
